...
class QuillEditor extends React.Component {
  ...
  insertImage = (e) => {
    ...
    ) 
    {
      ...
      console.log("this is from quilleditor ------->  ", this.props.user.email);
      const uploadTask = storage
        .ref(`${this.props.user.email}/images/${file.name}`)
        .put(file);
      uploadTask.on(
        "state_changed",
        (snapshot) => {},
        ...
        () => {
          var imageUrl = "";
          storage
            .ref(`${this.props.user.email}/images`)
            .child(file.name)
            .getDownloadURL()
            .then((url) => {
              imageUrl = url;
  ...
  insertVideo = (e) => {
    ...
    if (
      ...
      const file = e.currentTarget.files[0];
      const uploadTask = storage
        .ref(`${this.props.user.email}/videos/${file.name}`)
        .put(file);
      uploadTask.on(
        "state_changed",
        (snapshot) => {},
        ...
        () => {
          storage
            .ref(`${this.props.user.email}/videos`)
            .child(file.name)
            .getDownloadURL()
            .then((url) => {
              const quill = this.reactQuillRef.getEditor();
  ...
  insertFile = (e) => {
    ...
    if (
      ...
      const uploadTask = storage
      .ref(`${this.props.user.email}/files/${file.name}`)
      .put(file);
    uploadTask.on(
      "state_changed",
      (snapshot) => {},
